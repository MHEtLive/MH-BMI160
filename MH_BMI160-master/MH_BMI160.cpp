...
int8_t MH_BMI160::I2cInit(int8_t i2c_addr)
{
  Obmi160->id = i2c_addr;
  Obmi160->interface = BMI160_I2C_INTF;
  return MH_BMI160::I2cInit(Obmi160);
}

int8_t MH_BMI160::SPIInit()
{
  //SPI.begin();
  //pinMode(10,OUTPUT);
  //Obmi160->id = 0;
...
int8_t MH_BMI160::SPIInit(struct bmi160Dev *dev)
{
  return BMI160_OK;
}

int8_t MH_BMI160::I2cInit(struct bmi160Dev *dev)
{
...
int8_t MH_BMI160::softReset(struct bmi160Dev *dev)
{
  int8_t rslt=BMI160_OK;
  ...
  rslt = MH_BMI160::setRegs(BMI160_COMMAND_REG_ADDR, &data, 1, dev);
  delay(BMI160_SOFT_RESET_DELAY_MS);
  if (rslt == BMI160_OK){
    MH_BMI160::defaultParamSettg(dev);
  }  
  return rslt;
}
...
int8_t MH_BMI160::setSensConf()
{
  return MH_BMI160::setSensConf(Obmi160); 
}

int8_t MH_BMI160::setSensConf(struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  ...
  rslt = MH_BMI160::setAccelConf(dev);
  if (rslt == BMI160_OK) {
    rslt = MH_BMI160::setGyroConf(dev);
    if (rslt == BMI160_OK) {
      /* write power mode for accel and gyro */
      rslt = MH_BMI160::setPowerMode(dev);
      if (rslt == BMI160_OK)
        rslt = MH_BMI160::checkInvalidSettg(dev);
    }
  }
...
int8_t MH_BMI160::setAccelConf(struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  uint8_t data[2] = {0};
  rslt = MH_BMI160::checkAccelConfig(data, dev);
  if (rslt == BMI160_OK) {
    rslt = MH_BMI160::setRegs(BMI160_ACCEL_CONFIG_ADDR, &data[0], 1, dev);
    if (rslt == BMI160_OK) {
      dev->prevAccelCfg.odr = dev->accelCfg.odr;
      dev->prevAccelCfg.bw = dev->accelCfg.bw;
      delay(BMI160_ONE_MS_DELAY);
      rslt = MH_BMI160::setRegs(BMI160_ACCEL_RANGE_ADDR, &data[1], 1, dev);
      if (rslt == BMI160_OK){
...
int8_t MH_BMI160::checkAccelConfig(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt;

  /* read accel Output data rate and bandwidth */
  rslt = MH_BMI160::getRegs(BMI160_ACCEL_CONFIG_ADDR, data, 2, dev);
  if (rslt == BMI160_OK) {
    rslt = MH_BMI160::processAccelOdr(&data[0], dev);
    if (rslt == BMI160_OK) {
      rslt = MH_BMI160::processAccelBw(&data[0], dev);
      if (rslt == BMI160_OK)
        rslt = MH_BMI160::processAccelRange(&data[1], dev);
    }
  }
...
int8_t MH_BMI160::processAccelOdr(uint8_t *data,  struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t odr = 0;
...
int8_t MH_BMI160::processAccelBw(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t bw = 0;
...
int8_t MH_BMI160::processAccelRange(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t range = 0;
...
int8_t MH_BMI160::setGyroConf(struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t data[2]={0};

  rslt = MH_BMI160::checkGyroConfig(data, dev);
  
  if (rslt == BMI160_OK) {
    // Write output data rate and bandwidth 
    rslt = MH_BMI160::setRegs(BMI160_GYRO_CONFIG_ADDR, &data[0], 1, dev);
    if (rslt == BMI160_OK) {
      dev->prevGyroCfg.odr = dev->gyroCfg.odr;
      dev->prevGyroCfg.bw = dev->gyroCfg.bw;
      delay(BMI160_ONE_MS_DELAY);
      // Write gyro range 
      rslt = MH_BMI160::setRegs(BMI160_GYRO_RANGE_ADDR, &data[1], 1, dev);
      if (rslt == BMI160_OK)
...
int8_t MH_BMI160::checkGyroConfig(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt;

  /* read gyro Output data rate and bandwidth */
  rslt = MH_BMI160::getRegs(BMI160_GYRO_CONFIG_ADDR, data, 2, dev);
  if (rslt == BMI160_OK) {
    rslt = MH_BMI160::processGyroOdr(&data[0], dev);
    if (rslt == BMI160_OK) {
      rslt = MH_BMI160::processGyroBw(&data[0], dev);
      if (rslt == BMI160_OK)
        rslt = MH_BMI160::processGyroRange(&data[1], dev);
    }
  }
...
int8_t MH_BMI160::processGyroOdr(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t odr = 0;
...
int8_t MH_BMI160::processGyroBw(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t bw = 0;
...
int8_t MH_BMI160::processGyroRange(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t temp = 0;
  uint8_t range = 0;
...
int8_t MH_BMI160::setPowerMode(struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  rslt = MH_BMI160::setAccelPwr(dev);
  if (rslt == BMI160_OK){
    rslt = MH_BMI160::setGyroPwr(dev);
  }
  return rslt;
}

int8_t MH_BMI160::setAccelPwr(struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  uint8_t data = 0;
  ...
  if ((dev->accelCfg.power >= BMI160_ACCEL_SUSPEND_MODE) &&
    ...
    if (dev->accelCfg.power != dev->prevAccelCfg.power) {
      rslt = MH_BMI160::processUnderSampling(&data, dev);
      if (rslt == BMI160_OK) {
        /* Write accel power */
        rslt = MH_BMI160::setRegs(BMI160_COMMAND_REG_ADDR, &dev->accelCfg.power, 1, dev);
        /* Add delay of 5 ms */
...
int8_t MH_BMI160::processUnderSampling(uint8_t *data, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t temp = 0;
  uint8_t pre_filter = 0;

  rslt = MH_BMI160::getRegs(BMI160_ACCEL_CONFIG_ADDR, data, 1, dev);
  if (rslt == BMI160_OK) {
    if (dev->accelCfg.power == BMI160_ACCEL_LOWPOWER_MODE) {
      ...
      *data = temp | ((1 << 7) & BMI160_ACCEL_UNDERSAMPLING_MASK);
      ...
      if (rslt == BMI160_OK)
        ...
        rslt = MH_BMI160::setRegs(BMI160_INT_DATA_0_ADDR, &pre_filter, 2, dev);
    } else {
      if (*data & BMI160_ACCEL_UNDERSAMPLING_MASK) {
        ...
        *data = temp;
        /* Write data */
        rslt = MH_BMI160::setRegs(BMI160_ACCEL_CONFIG_ADDR, data, 1, dev);
      }
    }
  }
...
int8_t MH_BMI160::setGyroPwr(struct bmi160Dev *dev)
{
  int8_t rslt = 0;
  if ((dev->gyroCfg.power == BMI160_GYRO_SUSPEND_MODE) || (dev->gyroCfg.power == BMI160_GYRO_NORMAL_MODE)
    ...
    if (dev->gyroCfg.power != dev->prevGyroCfg.power) {
      /* Write gyro power */
      rslt = MH_BMI160::setRegs(BMI160_COMMAND_REG_ADDR, &dev->gyroCfg.power, 1, dev);
...
int8_t MH_BMI160::checkInvalidSettg( struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t data = 0;

  // read the error reg 
  rslt = MH_BMI160::getRegs(BMI160_ERROR_REG_ADDR, &data, 1, dev);
...
int8_t MH_BMI160::getSensorData(uint8_t type, int16_t* data)
{
  int8_t rslt=BMI160_OK;
  if (type==onlyAccel){
    ...
    rslt = MH_BMI160::getSensorData((BMI160_ACCEL_SEL | BMI160_GYRO_SEL),Oaccel, Ogyro, Obmi160);
    if(rslt == BMI160_OK){
      data[0]=Ogyro->x;
      data[1]=Ogyro->y;
      data[2]=Ogyro->z;
...
int8_t MH_BMI160::getAccelData( int16_t* data)
{
  int8_t rslt=BMI160_OK;
  rslt = getSensorData(BMI160_ACCEL_SEL, Oaccel, NULL, Obmi160);
  if(rslt == BMI160_OK){
...
int8_t MH_BMI160::getGyroData( int16_t* data)
{
  int rslt = BMI160_OK;
  rslt = getSensorData(BMI160_GYRO_SEL, NULL, Ogyro, Obmi160); 
  if(rslt == BMI160_OK){
...
int8_t MH_BMI160::getAccelGyroData( int16_t* data)
{
  int8_t rslt = BMI160_OK;
...
int8_t MH_BMI160::getSensorData(uint8_t select_sensor, struct bmi160SensorData *accel, struct bmi160SensorData *gyro,struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  uint8_t time_sel;
  uint8_t sen_sel;
  ...
  if (dev != NULL) {
    ...
    case eBmi160AccelOnly:
      ...
      if (accel == NULL)
        ...
        rslt = MH_BMI160::getAccelData(len, accel, dev);
      break;
    case eBmi160GyroOnly:
      ...
      if (gyro == NULL)
        ...
        rslt = MH_BMI160::getGyroData(len, gyro, dev);
      break;
    case eBmi160BothAccelAndGyro:
      ...
      if ((gyro == NULL) || (accel == NULL))
        ...
        rslt = MH_BMI160::getAccelGyroData(len, accel, gyro, dev);
      break;
    default:
...
int8_t MH_BMI160::getAccelData(uint8_t len, struct bmi160SensorData *accel, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t idx = 0;
  ...
  rslt = MH_BMI160::getRegs(BMI160_ACCEL_DATA_ADDR, data_array, 6 + len, dev);
...
int8_t MH_BMI160::getGyroData(uint8_t len, struct bmi160SensorData *gyro, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t idx = 0;
  ...
  if (len == 0) {
    /* read gyro data only */
    rslt = MH_BMI160::getRegs(BMI160_GYRO_DATA_ADDR, data_array, 6, dev);
    if (rslt == BMI160_OK) {
      /* Gyro Data */
      lsb = data_array[idx++];
      msb = data_array[idx++];
    ...
    rslt = MH_BMI160::getRegs(BMI160_GYRO_DATA_ADDR, data_array, 12 + len, dev);
    if (rslt == BMI160_OK) {
      /* Gyro Data */
      lsb = data_array[idx++];
      msb = data_array[idx++];
...
int8_t MH_BMI160::getAccelGyroData(uint8_t len, struct bmi160SensorData *accel, struct bmi160SensorData *gyro, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t idx = 0;
  uint8_t data_array[15] = {0};
...
int8_t MH_BMI160::getRegs(uint8_t reg_addr, uint8_t *data, uint16_t len, struct bmi160Dev *dev)
{
  ...
  if (dev == NULL) {
    ...
    if (dev->interface == BMI160_SPI_INTF){
      ...
      rslt = MH_BMI160::SPIGetRegs(dev, reg_addr, data, len);
    }else{
      rslt = MH_BMI160::I2cGetRegs(dev, reg_addr, data, len);
    }
    delay(1);
    if (rslt != BMI160_OK){
      rslt = BMI160_E_COM_FAIL;  
...
int8_t MH_BMI160::I2cGetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
...
int8_t MH_BMI160::setRegs(uint8_t reg_addr, uint8_t *data, uint16_t len, struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  uint8_t count = 0;
  //Null-pointer check
  if (dev == NULL) {
    ...
    if (dev->interface == BMI160_SPI_INTF){
      ...
      rslt = MH_BMI160::SPISetRegs(dev,reg_addr,data,len);
    }else{
      rslt = MH_BMI160::I2cSetRegs(dev,reg_addr,data,len);
    }
    delay(1);
...
int8_t MH_BMI160::I2cSetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
...
int8_t MH_BMI160::SPIGetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
...
int8_t MH_BMI160::SPISetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
  Serial.println("SPISetRegs");  
  return BMI160_OK;
}

int8_t MH_BMI160::setInt(int intNum)
{
  return setInt(Obmi160,intNum);  
}

int8_t MH_BMI160::setInt(struct bmi160Dev *dev, int intNum)
{
  int8_t rslt=BMI160_OK;
  if (dev == NULL){
    rslt = BMI160_E_NULL_PTR;
  ...
  rslt = MH_BMI160::setIntConfig(&intConfig, dev);
  return rslt;
}

int8_t MH_BMI160::setIntConfig(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
...
int8_t MH_BMI160::setAccelStepDetectInt(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
...
int8_t MH_BMI160::enableStepDetectInt(struct bmi160AccStepDetectIntCfg *stepDetectIntCfg, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t data = 0;
  uint8_t temp = 0;
...
int8_t MH_BMI160::setIntrPinConfig(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt;
...
int8_t MH_BMI160::configIntOutCtrl(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t temp = 0;
  uint8_t data = 0;
...
int8_t MH_BMI160::configIntLatch(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t temp = 0;
  uint8_t data = 0;
...
int8_t MH_BMI160::mapFeatureInterrupt(struct bmi160IntSettg *intConfig, struct bmi160Dev *dev)
{
  int8_t rslt;
...
int8_t MH_BMI160::configStepDetect(struct bmi160AccStepDetectIntCfg *stepDetectIntCfg, struct bmi160Dev *dev)
{
  int8_t rslt;
  uint8_t temp = 0;
...
int8_t MH_BMI160::setStepPowerMode(uint8_t model)
{
  return MH_BMI160::setStepPowerMode(model,Obmi160);
}

int8_t MH_BMI160::setStepPowerMode(uint8_t model,struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
...
int8_t MH_BMI160::setStepCounter()
{
  uint8_t step_enable = 1;//enable the step counter
  return MH_BMI160::setStepCounter(step_enable,Obmi160);
}

int8_t MH_BMI160::setStepCounter(uint8_t step_cnt_enable, struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  uint8_t data = 0;
...
int8_t MH_BMI160::readStepCounter(uint16_t *stepVal)
{
  return readStepCounter(stepVal,Obmi160);
}

int8_t MH_BMI160::readStepCounter(uint16_t *stepVal, struct bmi160Dev *dev)
{
  int8_t rslt = BMI160_OK;
  uint8_t data[2] = {0, 0};
  uint16_t msb = 0;
